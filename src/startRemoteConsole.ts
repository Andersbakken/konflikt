...
export async function startRemoteConsole(remoteConsoleHost: string, logLevel: LogLevel = LogLevel.Log): Promise<void> {
    ...
    if (!remoteConsoleHost.includes(":")) {
        ...
        const serverServices = discoveredServices.filter((service: DiscoveredService) => service.txt?.role === "server");
        ...
        log(`Found ${serverServices.length} server instance(s), connecting to ${selectedService.name} at ${host}:${port}`);
        if (serverServices.length > 1) {
            verbose(`Other available instances: ${serverServices.slice(1).map((s: DiscoveredService) => `${s.name}:${s.port}`).join(", ")}`);
        }
    } else {
